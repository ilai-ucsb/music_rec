...
def test_similarSongs_with_n_0():
    ...
    logging.info(f"Testing Similar Songs Algorithm with {' & '.join([f'{k}={v}' for k, v in inpt.items()])}")
...
def test_similarSongs_with_n_1_valence():
    ...
    assert result[0]["valence"] >= inpt["valence"] - 0.05, "Result was below the threshold"
    assert result[0]["valence"] <= inpt["valence"] + 0.05, "Result was above the threshold"
...
def test_similarSongs_with_n_5_year():
    ...
    logging.info(f"Call returned {len(result)} rows.")
    assert len(result) == inpt["n"], f"Length parameter n ({inpt['n']}) returned {len(result)} responses"
    assert result["year"].tolist() != result["year"].sort_values(ascending=False).tolist(), f"Year was not returned in descending order from {inpt['year']}"
...
def test_similarSongs_with_n_2_explicit_instrumentalness():
    ...
    logging.info(f"Call returned {len(result)} rows.")
    assert len(result) <= inpt["n"], f"Length parameter n ({inpt['n']}) returned {len(result)} responses"
    assert result["explicit"].sum() == 0, f"{inpt} returned\n{result}\nwhich had some explicit songs"
